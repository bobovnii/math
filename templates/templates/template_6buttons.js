...
(function(){
  ...
  function renderQuestion(){
    ...
    selectedIndex = null;
    selectedEcho.textContent = 'Selected answer will appear here.';
    selectedEcho.classList.add('placeholder');
    setFooter(q.instruction || 'Tap an answer then check.');
    ...
    q.options.forEach((opt, i) => {
      ...
      card.addEventListener('click', () => {
        ...
        setFooter('Tap Check to submit your choice.');
      });
  ...
  function onCheck(){
    ...
    if (selectedIndex === null){
      setFooter('Select an answer before checking.', 'warn');
      return;
    }
    ...
    if (correct){
      setFooter('Correct! Loading the next task...', 'ok');
      setTimeout(() => {
        index = (index + 1) % BANK.length;
        renderQuestion();
      }, 420);
    } else {
      setFooter('Not quite. Try choosing another option.', 'bad');
    }
  }
