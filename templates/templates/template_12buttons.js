...
(function(){
  ...
  function handleSymbol(symbol){
    if (symbol.toLowerCase() === 'clear'){
      currentInput = "";
      updateDisplay();
      setFooter('Cleared. Start again.', 'warn');
      return;
    }
    const status = canAppend(symbol);
    if (status !== 'ok'){
      const message = status === 'comma'
        ? 'Only one decimal comma per answer.'
    ...
    currentInput += symbol;
    updateDisplay();
    setFooter('Keep typing or tap Check.');
  }
  ...
  function onCheck(){
    ...
    if (!currentInput.length){
      setFooter('Type an answer before checking.', 'warn');
      return;
    }
    ...
    if (correct){
      setFooter('Correct! Next task coming up...', 'ok');
      setTimeout(() => {
        index = (index + 1) % BANK.length;
        renderQuestion();
      }, 420);
    } else {
      setFooter('Not quite. Check your input.', 'bad');
    }
  ...
  function updateDisplay(){
    if (!currentInput.length){
      answerDisplay.textContent = 'Type the answer using the keypad';
  ...
  function renderQuestion(){
    const q = BANK[index];
    titleEl.textContent = cfg.title || 'Quiz';
    questionPrompt.textContent = q.prompt;
    questionInstruction.textContent = q.instruction;
    currentInput = "";
    updateDisplay();
    setFooter('Use the keypad and press "Check".');
  }
})();
