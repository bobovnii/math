...
(function(){
  ...
  function init(){
    ...
    titleEl.textContent = cfg.title || 'Formula Builder';
  ...
  function buildSymbols(){
    ...
    baseTokens.forEach(symbol => {
      ...
      btn.addEventListener('click', () => {
        ...
        setFooter('Keep building the formula...', '');
      });
  ...
  function onCheck(){
    if (!builtTokens.length){
      setFooter('Add symbols before checking.', 'warn');
      return;
    }
    ...
    if (isCorrect){
      setFooter('Correct! That matches the formula.', 'ok');
    } else {
      setFooter('Not yet. Check the order and parentheses.', 'bad');
    }
  }

  function updateDisplay(){
    if (!builtTokens.length){
      formulaDisplay.textContent = cfg.placeholder || 'Tap symbols to build the formula';
    } else {
